...
- **Step 1 — paste this whole file (or upload it and say "follow this").** The assistant saves the
...
- **Live knockout board** — a new "Knockout rounds — live" section renders the Round of 16,
  Quarterfinals, Semifinals, and Final as real result rows (teams + scores, your-pick badges),
  not just an upcoming-fixtures list. Each match's teams are the winners of its two feeder
  matches, so the board fills in automatically as rounds complete.
- **Every round now scores from live results** — a shared bracket topology (`KO_FEED`) plus
...
- **Sticky left side-navigation rail** (glass panel, emoji + label links, one per top-level
  section) with an `IntersectionObserver` **scrollspy** that highlights the current section, smooth
  anchor jumps, and full keyboard/focus support. It is **navigation only** — no progress bar.
- **Top "last refreshed" indicator** — a live `Updated <REFRESHED>` chip in the header that mirrors
  the footer timestamp, so the last sync time is visible without scrolling.
- Both are part of the **verbatim render engine** below — they need no per-entrant data and are
  reproduced automatically. Nothing about the picks, scoring, or data model changed from v6.

### Side-navigation rail — design notes (already implemented in the generator)

- A `.glass` `<nav class="rail">` in a **two-column shell** (`grid-template-columns:186px
  minmax(0,1fr)`): rail on the left, page content on the right. The rail is `position:sticky`.
- **No horizontal scroll for the bracket:** `.content{min-width:0}` lets the content column shrink
  so the bracket keeps its *own* internal `overflow-x` (it never forces a page/bracket scrollbar);
  `body{overflow-x:hidden}` is the final guard.
- **Responsive:** at `≤1200px` the shell collapses to one column and the rail becomes a tap-open
  **`📑 Contents ☰`** drawer (`.rail.open .links`); links close it on click.
- **Active state** uses the mint/teal accent (`--teal`) as a 2px left border plus a faint glass
  wash — matching the existing design system. Scrollspy `rootMargin:'-45% 0px -50% 0px'`.
- Section anchors are the `#intro` hero and each `.shead[id]` (`sec-standing`, `sec-scorecard`,
  `sec-r32`, `sec-r16`, `sec-news`, `sec-bracket`, `sec-finalfour`, `sec-story`, `sec-scoring`).
...
1. Save the Python generator in the code block below to `working/build_dashboard.py`, **verbatim** —
   do not restyle, re-order, rename, or "improve" anything below the USER DATA banner.
2. Reply: "Template ready — drop your filled-in SLED World Cup 2026 bracket Excel and I'll build
...
2. **Do not edit the RENDER ENGINE** (everything below the USER DATA banner) — it produces the exact
   ...
   reading modes, scorecard, KPIs, the sticky left side-navigation rail (with scrollspy), the top
   "last refreshed" indicator, and layout. All on-screen copy derives from the data.
...
**Step 3 (after the dashboard is delivered) — offer a schedule; do NOT set one up without asking:**
This is a light UPDATER, not a rebuild — the picks never change, so a run only re-pulls the latest
FIFA results and re-renders from the saved generator.
1. Briefly explain Cowork scheduled tasks (1–2 sentences): Cowork can run a task on its own on a
   schedule you set — here, re-pulling the latest results and re-rendering this dashboard — so it
   stays current without you asking.
2. Ask: do you want it to auto-update through the end of the World Cup? What is your timezone?
3. **Only run on days that actually have matches — skip the off-days.** Set up ONE recurring task
   that runs on each match date, timed shortly after that day's LAST match is final (so results are
   in), converted to the user's timezone; run through the Final on July 19, then stop (on each run,
   if today is past July 19, 2026, delete the task; if self-deletion is unavailable, tell them to
   pause/remove it after the final).

   2026 World Cup remaining schedule — last match kickoff, Pacific Time:
     Jul 2  Switzerland vs Algeria ...... 8:00 PM PT
     Jul 3  Colombia vs Ghana ........... ~6:30 PM PT
     Jul 4  last Round-of-16 match ...... ~2:00 PM PT
     Jul 5  last Round-of-16 match ...... ~5:00 PM PT
     Jul 6  last Round-of-16 match ...... ~5:00 PM PT
     Jul 7  last Round-of-16 match ...... ~5:00 PM PT
     Jul 9  Quarterfinal ................ ~5:00 PM PT
     Jul 10 Quarterfinal ................ ~5:00 PM PT
     Jul 11 Quarterfinal ................ ~6:00 PM PT
     Jul 14 Semifinal ................... ~12:00 PM PT
     Jul 15 Semifinal ................... ~12:00 PM PT
     Jul 19 Final ....................... 12:00 PM PT
     NO matches Jul 8, 12, 13, 16, 17, 18 — do not schedule runs on those days.
...
Honesty: show only what the sheet and verified results contain; if the file can't be read or a
result can't be verified, say so plainly.

---

## The generator (save verbatim as `build_dashboard.py`)
